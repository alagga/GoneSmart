...
Match native playlist title **font size and typeface**, title/metadata distinction, margins, item height, overflow/ripple, row spacing and background. Do not add artificial dividing lines to native Playlists/Add rows. Do not infer the whole UI from a single GMMP skin: verify the current theme, another accent palette and a structurally different view mode before calling broad skin parity complete.

**Current experimental note, 2026-09-25:** screenshots showed an undersized synthetic playlist font, heavy filled folder icon and gray Add-picker overlay. A native compact metadata TextView measured **30 px** on a **144 px** row, while the screenshot's visible title appeared closer to **45 px**. The latest `ada785a` build attempts proportionate correction, thin outline folders, genuine window surface, and safer overlay lifecycle; **its real-device result is still pending**. Do not freeze 45 px as a universal size or claim the overlay/fragment crashes resolved without a new device pass.
