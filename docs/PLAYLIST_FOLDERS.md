...
**Still unfinished:** picker creation-only behavior needs interception at the confirmed `fo3.invoke` callback and verified native result/event handling; do not skip GMMP's underlying creation transaction or simulate success. The picker flash may be caused by its native post-create auto-close; preserve browser coverage through a real foreground transition while verifying create-only semantics. The native creation probe was installed in commit `b3b076f`, but the provided test log does not show `NATIVE CREATE PROBE` lines, so verify the APK/commit before interpreting their absence as a failed hook. The physical-folder creation guard remains active pending destination/DB confirmation.
