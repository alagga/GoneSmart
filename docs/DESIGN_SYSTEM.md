...
**Breadcrumb interaction refinement (2026-09-25):** never rebuild an unchanged path row merely because the playlist list refreshed or a destination was selected. Every segment, **including the current last folder**, must have the same native touch/ripple feedback when tapped; tapping the current folder is a no-op after showing feedback. Delay ancestor navigation until the next animation frame to avoid removing the pressed-state view before its ripple renders.

**GMMP native animation parity request (2026-09-25):** when GMMP inserts a newly created playlist into its original list, it animates that addition. The GoneSmart folder browser must not make the new row appear instantaneously. Mirror the actual native RecyclerView ItemAnimator's add/move durations and default interpolation for rows inserted into the currently displayed folder; preserve the original GMMP row XML, theme and click behavior. Skip animation on first attach and folder navigation. Device-verify against GMMP's original insert animation, and inspect the runtime native animator class before claiming exact equivalence.

**Native Files quickNav timing (2026-09-25):** at first Add-picker entry the actual native Files quickNav may not yet have been instantiated. Use the measured GMMP 4.2.0 quickNav-to-native-playlist-title ratio as an interim fallback, dynamically derived from a freshly bound playlist title, and persist/replace it with the real native sample once the native Files tab is visited. An unchanged breadcrumb must not be rebuilt or invalidate a pending scroll; navigation always reveals the newest/rightmost segment. Even a two-segment breadcrumb should have Android's native short-list edge stretch when dragged past its ends.

**2026-09-26 native-mechanics requirement:** prefer *directly running* GMMP's own RecyclerView/ItemAnimator/EdgeEffectFactory rather than hand-drawn approximations. GoneSmart's path header now uses an actual AndroidX RecyclerView (matching installed GMMP 4.2.0 v1.4.0), copies the native Files-tab quickNav EdgeEffectFactory and overscroll settings once observed and retains native sampled typography/ripples. The playlist insertion path creates a separate instance of the live native playlist RecyclerView ItemAnimator's exact concrete class and runs its real animateAdd/animateMove methods on the injected native XML row views. A missing native custom constructor can trigger a logged fallback. Test on device; do not describe an untested transition as pixel-perfect.
